...
describe AtmPrinter do
  ...
  context 'Deposits' do
    it 'After a deposit, it should show this in the account history in the correct format with a timestamp of the date it was made on' do
      subject.update_account_history("#{Date.today.strftime('%d-%m-%Y')} || 5 || || 5")
      output = "date || credit || debit || balance \n#{Date.today.strftime('%d-%m-%Y')} || 5 || || 5 \n"
      expect(subject.print_transaction_history).to eq(output)
    end
  ...
  context 'Withdrawals' do
    it 'After a withdrawal, it should show this in the account history in the correct format with a timestamp of the date it was made on' do
      subject.update_account_history("#{Date.today.strftime('%d-%m-%Y')} || 5 || || 5")
      subject.update_account_history("#{Date.today.strftime('%d-%m-%Y')} || || 5 || 0")
      output = "date || credit || debit || balance \n#{Date.today.strftime('%d-%m-%Y')} || || 5 || 0 \n#{Date.today.strftime('%d-%m-%Y')} || 5 || || 5 \n"
      expect(subject.print_transaction_history).to eq(output)
